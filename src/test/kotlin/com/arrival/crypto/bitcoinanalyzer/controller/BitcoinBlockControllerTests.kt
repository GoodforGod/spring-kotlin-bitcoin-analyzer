...
class BitcoinBlockControllerTests(@Autowired val restTemplate: TestRestTemplate) : Assertions() {
    ...
    fun `valid sub hash is calculated`() {
        val entity = restTemplate.getForEntity<String>("$path?from=646940&to=646941")
    ...
    fun `from less than 0 is rejected`() {
        val entity = restTemplate.getForEntity<String>("$path?from=-1&to=5")
        assertEquals(HttpStatus.BAD_REQUEST, entity.statusCode)
    }

    @Test
    fun `to less than from is rejected`() {
        val entity = restTemplate.getForEntity<String>("$path?from=5&to=3")
        assertEquals(HttpStatus.BAD_REQUEST, entity.statusCode)
    }

    @Test
    fun `to more than integer is rejected`() {
        val entity = restTemplate.getForEntity<String>("$path?from=64694000000&to=6469410000000")
        assertEquals(HttpStatus.BAD_REQUEST, entity.statusCode)
    }

    @Test
    fun `range more than 100 is rejected`() {
        val entity = restTemplate.getForEntity<String>("$path?from=100&to=300")
        assertEquals(HttpStatus.BAD_REQUEST, entity.statusCode)
    }
}